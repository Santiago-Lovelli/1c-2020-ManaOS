...
void EsperarClientes(){
	...
	while(1){
		...
		pthread_t* hiloDeAtencion = malloc(sizeof(pthread_t)); //free
		if (pthread_create(hiloDeAtencion, NULL, AtenderCliente, cliente) != 0){
...
void* AtenderCliente(void* cliente) {
	while(1){
		HeaderDelibird headerRecibido =  Serialize_RecieveHeader((int)cliente);
...
int tratarMensaje (d_message tipoMensaje, void *paquete){
	...
	if (resultado){
		...
		int tam = tamanioDeMensaje(tipoMensaje, unMensaje);
		memcpy(resultado->donde, unMensaje, tam);//guardarMensajeEnMemoria()
...
void ConfigInit(){
	t_config* configCreator = config_create("/home/utnso/workspace/Broker2/Broker.config");
...
void MemoriaPrincipalInit(){
	...
	estructura = malloc (sizeof (estructuraAdministrativa));
	...
	fecha = temporal_get_string_time();
	string_append(&estructura->tiempo, fecha);
	string_append(&estructura->ultimaReferencia, fecha);
...
int tamanioDeMensaje(d_message tipoMensaje, void * unMensaje){
	uint32_t tam, tamPuntos;
	void * punteroMovil;
	switch(tipoMensaje){
	case d_NEW_POKEMON:
		memcpy(&tam, unMensaje, sizeof(uint32_t));
		return (tam * sizeof(char) + sizeof(uint32_t) * 4);
	case d_CATCH_POKEMON:
		memcpy(&tam, unMensaje, sizeof(uint32_t));
		return (tam * sizeof(char) + sizeof(uint32_t) * 3);
	case d_GET_POKEMON:
		memcpy(&tam, unMensaje, sizeof(uint32_t));
		return (tam * sizeof(char) + sizeof(uint32_t));
	case d_APPEARED_POKEMON:
		memcpy(&tam, unMensaje, sizeof(uint32_t));
		return (tam * sizeof(char) + sizeof(uint32_t) * 4);
	case d_CAUGHT_POKEMON:
	...
	case d_LOCALIZED_POKEMON:
		punteroMovil = malloc(sizeof(void*));
		memcpy(&tam, unMensaje, sizeof(uint32_t));
		punteroMovil = unMensaje + sizeof(uint32_t) + sizeof(char) * tam;
		memcpy(&tamPuntos, punteroMovil, sizeof(uint32_t));
		free (punteroMovil);
		return (tam * sizeof(char) + sizeof(uint32_t)*2 * tamPuntos + sizeof(uint32_t) * 2);
	default:
...
int reemplazar (d_message tipoMensaje, void* mensaje){
	...
	log_info (LOGGER_OBLIGATORIO, "No hay metodo de reemplazo asignado");
	return -1;
...
void * cargarMensajeAGuardar(d_message tipoMensaje, void *paquete, uint32_t* id) {
	...
	case d_NEW_POKEMON:
		...
		unNombre = malloc(retornoNew->largoDeNombre);
		memcpy(unNombre, &pokemon, retornoNew->largoDeNombre);
		retornoNew->nombrePokemon = unNombre;
	...
	case d_CATCH_POKEMON:
		...
		unNombre = malloc(retornoCatch->largoDeNombre);
		memcpy(unNombre, &pokemon, retornoCatch->largoDeNombre);
		retornoCatch->nombrePokemon = unNombre;
	...
	case d_GET_POKEMON:
		...
		unNombre = malloc(retornoGet->largoDeNombre);
		memcpy(unNombre, &pokemon, retornoGet->largoDeNombre);
		retornoGet->nombrePokemon = unNombre;
		retorno = retornoGet;
		break;
	case d_APPEARED_POKEMON:
		retornoAppeared = malloc(sizeof(appearedEnMemoria));
		...
		unNombre = malloc(retornoAppeared->largoDeNombre);
		memcpy(unNombre, &pokemon, retornoAppeared->largoDeNombre);
		retornoAppeared->nombrePokemon = unNombre;
	...
	case d_LOCALIZED_POKEMON:
		...
		retornoLocalized->largoDeNombre = string_length(pokemon);
		unNombre = malloc(retornoLocalized->largoDeNombre);
		memcpy(unNombre, &pokemon, retornoLocalized->largoDeNombre);
		retornoLocalized->nombrePokemon = unNombre;
